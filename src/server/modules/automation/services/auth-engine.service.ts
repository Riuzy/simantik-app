...
export class AuthEngine {
  ...
  private buildBrowserLogin(varName: string, email: string, password: string, loginPageUrl: string, timeout: number): string {
    return [
      ...
      `    ], 'auth:email', ${email}, { timeout: ${timeout}, requireVisible: true, requireEnabled: true });`,
      `    await __fillElement(${varName}, [`,
      ...
      `    ], 'auth:password', ${password}, { timeout: ${timeout}, requireVisible: true, requireEnabled: true });`,
      `    await __clickElement(${varName}, [`,
